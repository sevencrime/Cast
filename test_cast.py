...
class Test_Vap_Data():
    ...
    def test_check_VAP_API_channel(self, conn):
    ...
    # 数据库和接口地址环境要对应
    def test_check_VAP_API_appChannel(self, conn):

        api_start_time = 1679068800
        api_end_time = 1679155199
        api_douyin_no = 'wusu868688'
        ...
        room_ids_last_cycle_sql = f'''
            ...
            FROM vap_douyin_live_rooms
            WHERE douyin_no = '{api_douyin_no}'
                AND start_time >= {api_start_time} - 31*86400
                AND start_time <= {api_end_time} - 86400
                AND status <> 1
        '''
        room_ids_last_cycle_df = self.execute_sql_stime(room_ids_last_cycle_sql, conn)
        room_ids_last_cycle = room_ids_last_cycle_df['room_id'].tolist()
        ...
        body = {
            ...
            "end_time": api_end_time,
            "metrics":["lp_gpm"],
            "room_ids": list(tuple(room_ids)),
            "room_ids_last_cycle": tup_room_ids_last_cycle,
            "compare_days":30,
            "sort_field":"this_value",
            "is_desc":"desc"
        }
        print(json.dumps(body))
        resp = requests.post(self.cast_api_dev + '/vap/api/app_channel', json.dumps(body)).json()
        # print(resp)
        ...
        pg_sql_1 = f'''
            WITH channel_analysis_room AS (
                SELECT * FROM (
                    SELECT *, rank() OVER(PARTITION BY room_id ORDER BY etl_flag DESC) rn
            ...
            this_period AS (
                SELECT douyin_no, first_channel_code, first_channel_name, second_channel_code, second_channel_name
                    ...
                    , SUM(gmv) AS gmv
                    , round(AVG(NULLIF(gpm, 0)), 2) AS gpm
                    , SUM(jl_watch_cnt) AS jl_watch_cnt
                FROM channel_analysis_room
                WHERE room_id IN {tup_room_ids if len(tup_room_ids) > 0 else "(' ')"}
                    AND start_time >= {api_start_time}
                    AND start_time <= {api_end_time}
                GROUP BY douyin_no, first_channel_code, first_channel_name, second_channel_code, second_channel_name
            ),
            last_period AS (
                SELECT douyin_no, first_channel_code, first_channel_name, second_channel_code, second_channel_name
                    , round(AVG(live_watch_cnt), 2) AS live_watch_cnt
                    , round(AVG(gmv), 2) AS gmv
                    , round(AVG(NULLIF(gpm, 0)), 2) AS gpm
                    , round(AVG(jl_watch_cnt), 2) AS jl_watch_cnt
                FROM (
                    SELECT douyin_no, first_channel_code, first_channel_name, second_channel_code, second_channel_name
                        , SUM(live_watch_cnt) AS live_watch_cnt
                        , SUM(gmv) AS gmv
                        ...
                        , SUM(jl_watch_cnt) AS jl_watch_cnt
                    FROM channel_analysis_room
                    WHERE room_id IN {tup_room_ids_last_cycle if len(tup_room_ids_last_cycle) > 0 else "(' ')"}
                        -- AND start_time >= {api_start_time} - 86400* {body['compare_days']}
                        -- AND start_time <= {api_start_time} - 86400
                    GROUP BY douyin_no, first_channel_code, first_channel_name, second_channel_code, second_channel_name, to_char(to_timestamp(start_time), 'YYYY-MM-DD')
                ) t GROUP BY douyin_no, first_channel_code, first_channel_name, second_channel_code, second_channel_name
            ),
            tmp1 AS (
                SELECT *
                    , (CASE WHEN last_rate = 0 THEN 0 ELSE round((this_rate - last_rate)/last_rate * 100, 2) END) AS diff_last_rate
                FROM (
                    SELECT
                        ...
                        , COALESCE(A.first_channel_code, B.first_channel_code) AS first_channel_code
                        , COALESCE(A.first_channel_name, B.first_channel_name) AS first_channel_name
                        , COALESCE(A.second_channel_code, B.second_channel_code) AS second_channel_code
                        , COALESCE(A.second_channel_name, B.second_channel_name) AS second_channel_name
                        , A.{field} AS this_value
                        , B.{field} AS "last_value"
                        , CASE WHEN B.{field} = 0 THEN 0 ELSE round((A.{field} - B.{field})/B.{field}::NUMERIC * 100, 2) END AS diff_last_value
                        , (CASE WHEN SUM(A.{field}) OVER(PARTITION BY A.douyin_no) = 0 THEN 0
                        ELSE round(A.{field} / SUM(A.{field}) OVER(PARTITION BY A.douyin_no)::NUMERIC * 100, 2)
                        END ) AS this_rate
                        , (CASE WHEN SUM(B.{field}) OVER(PARTITION BY A.douyin_no) = 0 THEN 0
                        ELSE round(B.{field} / SUM(B.{field}) OVER(PARTITION BY A.douyin_no)::NUMERIC * 100, 2)
                        END ) AS last_rate
                    FROM this_period A
                    FULL JOIN last_period B ON A.douyin_no = B.douyin_no AND A.first_channel_code = B.first_channel_code AND A.second_channel_code = B.second_channel_code
                ) t
            ),
            tmp2 AS (
                SELECT *
                ...
                FROM (
                    SELECT *
                        , (CASE WHEN SUM(this_value::NUMERIC) OVER(PARTITION BY douyin_no) = 0 THEN 0
                        ELSE round(this_value / SUM(this_value::NUMERIC) OVER(PARTITION BY douyin_no)::NUMERIC * 100, 2)
                        END ) AS this_rate
                        , (CASE WHEN SUM("last_value"::NUMERIC) OVER(PARTITION BY douyin_no) = 0 THEN 0
                        ELSE round("last_value" / SUM("last_value"::NUMERIC) OVER(PARTITION BY douyin_no)::NUMERIC * 100, 2)
                        END ) AS "last_rate"
                    FROM (
                        SELECT douyin_no, first_channel_code, first_channel_name
                            , '' AS second_channel_code
        ...
        df2 = self.execute_sql_stime(pg_sql_1, conn)
...
if __name__ == '__main__':
    ...
    pytest.main([
                '-s'
                , '-v'
                , 'test_cast.py::Test_Vap_Data::test_check_VAP_API_appChannel'
                # , '--alluredir'
                # , report_xml
                # , '--clean-alluredir'
                ])
